...
def get_invoice(invoice_id: uuid.UUID, session: SessionDep) -> InvoiceRead:
    ...
    return InvoiceRead.from_invoice(invoice, customer_name=customer.name if customer else None)
...
def list_invoices(session: SessionDep, limit: int = 50, offset: int = 0) -> list[InvoiceRead]:
    ...
    return [InvoiceRead.from_invoice(i, customer_name=names.get(i.customer_id)) for i in invoices]
