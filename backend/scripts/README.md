...
| `prepare_review.py` | **Before a mentor visits.** Stage one open dispute and one active promise so the queue has something worth reading; `--check` reports state without changing it |
