...
export default function LiveInvoicesPage() {
  ...
  if (!merchant) return <LiveSignInPrompt what="Your invoice ledger" />;
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-4">Recovery queue</p><h1 className="text-2xl font-semibold tracking-[-0.03em] text-ink sm:text-[1.75rem]">Invoices that need attention</h1><p className="mt-1.5 text-sm leading-6 text-ink-3">Open an invoice for its conversation, payments, promises, disputes, and audit trail.</p></div><button type="button" onClick={downloadTemplate} className="rounded-lg border border-line px-4 py-2 text-sm">Download CSV template</button></div>
    <section id="import" className="rounded-xl border border-line bg-panel p-5">
